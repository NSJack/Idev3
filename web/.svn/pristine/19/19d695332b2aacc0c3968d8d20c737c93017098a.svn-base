...
function encrypt($password){
    return hash('sha512','SALT_abc123' . $password);
}

/**
 * verify 对比明文和密文 ，判断密码是否正确
 * @author 姜君
 * @param string $clear 用户输入的明文密码
 * @param string $cipher 一般是从数据库中取出的密文
 * @return bool 
 */

function pwVerify($clear,$cipher){
    //先把明文加密后，再判断是否和密文相等
