$(function() {
		...
		$("#mobile").blur(function(){
			...
			$.ajax({
				...
				success:function (d) {
					...
					}else if(obj.chk=='中途被修改'){
						info('mobile','red',"该手机号和接收验证码的手机号，不一致。",true);
						chkArr['mobile']=3;
		...
		$("#regForm input").focus(function(){
			id=$(this).attr('id');
			info(id,'red',"",false);
			if(id=='mobile'){//手机号输入框，得到焦点后，就让获取验证码按钮失效
				$("#btnCode").attr("disabled","disabled");
			}
			$("#submit").attr("disabled","disabled");//让提交按钮失效
			$("#test").html("当前的校验数组为："+JSON.stringify(chkArr));
		})//得到焦点事件结束

		//统一加一次失焦事件
		$("#regForm input").blur(function(){
			chkOk=true;//校验总结果，默认成功
			//检查每个校验结果
			$.each(chkArr,function(key,val){
				if (val!=2) {
					chkOk=false;//只要有一个校验不通过，总结果就不通过
					return;
				}
			})
			//如果通过所有校验，就激提交按钮
